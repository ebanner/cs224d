...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        if node.isLeaf: # base base
            node.hActs1 = self.L[:, [node.word]]
        else: # recursive case
             # 
            # Tell left and right nodes to forwardProp already!
            left_cost, left_total = self.forwardProp(node.left)
            right_cost, right_total = self.forwardProp(node.right)

            cost += left_cost + right_cost
            total += left_total + right_total

            # Now left.hActs1 and right.hActs2 are available!
            #
